  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        fontWeight: 600ize: 14, cursor: 'pointer', whiteSpace: 'nowrap',
      }}
      ...
      {options.map((o) => {
        ...
        return (
          <button key={o} onClick={() => onChange(o)}
            style={{
              padding: '5px 12px', borderRadius: 999ize: 12,
  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 18ize: 13, color: t.ink2 }}>
      ...
      {trades.length === 0 ? (
        <div style={{
          ...
          fontFamily: t.seriftyle: 'italic'ize: 18, color: t.ink3,
        }}>
    ...
    <div style={{ borderTop: `1px solid ${t.rule}` }}>
      {trades.map((tr) => (
        <button key={tr.id} onClick={() => onNavigate('detail', tr.id)}
          ...
          <span style={{ fontFamily: t.seriftyle: 'italic', color: t.ink3ize: 13 }}>
            #{tr.id}
          </span>
          <span style={{ color: t.ink2ize: 13 }}>{tr.date.slice(5)}</span>
          <span style={{ fontFamily: t.serifize: 18, fontWeight: 500, color: t.ink }}>
            {tr.pair}
          </span>
          <span style={{
      ...
      {trades.map((tr) => (
        <button key={tr.id} onClick={() => onNavigate('detail', tr.id)}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <span style={{ fontFamily: t.serifize: 22, fontWeight: 500, color: t.ink }}>
              {tr.pair}
            </span>
            <Pnl t={t} pl={tr.pl} />
          </div>
          <div style={{ fontFamily: t.seriftyle: 'italic'ize: 14, color: t.ink2 }}>
